...
assert.equal(first.settings.hidden, true);
first.accept.click();
...
assert.equal(run({ value: first.storage.value }).banner.hidden, true, 'remember acceptance on the next visit');
const refused = run(); refused.refuse.click();
assert.equal(refused.location.reloaded, undefined);
assert.equal(refused.banner.hidden, true);
assert.equal(refused.appended.length, 0);
assert.equal(run({ value: refused.storage.value }).banner.hidden, true, 'remember refusal on the next visit');
